...
void idt_default_handler(idt_regs_t* regs) { }
...
void idt_init()
{
    ...
    _idtr      = ((uint32_t)&idtr) - VIRT_KERNEL;
    idtr.base  = ((uint32_t)&idt) - VIRT_KERNEL;
    idtr.limit = (IDT_COUNT * sizeof(idt_entry_t)) - 1;    

    _cli();
    _idt_flush();
    irq_register(IRQ7, idt_default_handler);
    debug_ok("Initialized IDT - COUNT: %d, LIST: 0x%8x, IDTR: 0x%8x", IDT_COUNT, idtr.base, _idtr);
}
...
void _isr_handler(idt_regs_t* regs)
{
    panic(regs->err, regs, true);
    asm volatile("cli; hlt");
}

void _irq_handler(idt_regs_t* regs)
{
    if (regs->irq >= IRQ0 + 8) { port_outb(0xA0, 0x20); }
    port_outb(0x20, 0x20);

    if (idt_handlers[regs->irq] != NULL) { idt_handlers[regs->irq](regs); return; }
    if (regs->irq > IRQ0) { debug_error("Unhandled interrupt: 0x%2x", regs->irq - IRQ0); }
}