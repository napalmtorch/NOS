...
{ 
    if (PROCMGR.current->id == 0) { return heap_alloc(&HEAP_USER, size, false, HEAPTYPE_USED); }
    return heap_alloc(&HEAP_USER, size, false, HEAPTYPE_USED); 
}

void* malloc_c(size_t size) 
{ 
    if (PROCMGR.current->id == 0) { return heap_alloc(&HEAP_USER, size, true, HEAPTYPE_USED); }
...
void free(void* ptr)
{
    ...
    entry = heap_locate(&HEAP_KERNEL, ptr);
    if (entry != NULL) { heap_free(&HEAP_KERNEL, ptr); return; }

    entry = NULL;
    entry = heap_locate(&HEAP_USER, ptr);
    if (entry != NULL) { heap_free(&HEAP_USER, ptr); return; }

    debug_error("Unable to locate and free pointer 0x%8x", ptr);
}

void kfree(void* ptr) { heap_free(&HEAP_KERNEL, ptr); }
